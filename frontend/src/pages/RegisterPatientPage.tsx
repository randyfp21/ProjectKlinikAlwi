...
export const RegisterPatientPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col justify-between p-3 sm:p-6 lg:p-8 font-sans transition-colors">
      {/* Top Header Navigation Bar */}
      <div className="w-full max-w-2xl mx-auto flex items-center justify-between py-2 px-1 mb-2 sm:mb-4">
        <Link
          to="/"
          className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-medium flex items-center gap-1.5 transition"
        >
      ...
      <div className="w-full max-w-2xl mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 sm:p-8 shadow-xl space-y-6 transition-colors my-auto">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">{t('registerTitle')}</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto leading-relaxed">{t('registerSubtitle')}</p>
        </div>

        {success && (
          <div className="p-4 rounded-2xl bg-emerald-500 text-white font-bold text-xs shadow-lg flex items-center justify-center gap-2 animate-bounce">
            <CheckCircle2 className="w-5 h-5 shrink-0" /> Account created successfully! Redirecting...
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          {/* NIK Warning Banner */}
          <div className="p-3.5 rounded-2xl bg-sky-500/10 border border-sky-500/20 text-sky-700 dark:text-sky-300 font-semibold text-[11px] leading-relaxed">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
            {/* NIK KTP Field - Full Width on Mobile & Tablet */}
            <div className="sm:col-span-2 space-y-1">
              <div className="flex flex-wrap items-center justify-between gap-1 mb-1">
                ...
                {formData.national_id.length === 16 ? (
                  ...
                  <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/30 text-[10px] font-bold">
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full p-3 pr-10 bg-slate-50 dark:bg-slate-800 border rounded-xl font-mono font-bold text-sky-600 dark:text-sky-400 focus:outline-none transition ${
                    formData.national_id.length === 16
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-slate-100 focus:outline-none focus:border-teal-500 transition"
              />
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <div className="flex items-center">
                <span className="px-3.5 py-3 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-bold font-mono text-xs rounded-l-xl border border-r-0 border-slate-200 dark:border-slate-700 select-none">
                  +62
                </span>
                <input
                  type="text"
                  ...
                  className="w-full p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-r-xl text-slate-900 dark:text-slate-100 font-mono font-semibold focus:outline-none focus:border-teal-500 transition"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-slate-100 focus:outline-none transition"
              >
                <option>Male</option>
            ...
            {/* Blood Type & Allergy - 2 Columns Row */}
            <div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-xl bg-teal-600 hover:bg-teal-500 text-white font-bold text-xs shadow-lg shadow-teal-600/30 flex items-center justify-center gap-2 transition cursor-pointer mt-4"
          >
      ...
      <div className="py-2 text-center text-[11px] text-slate-400">
        &copy; {new Date().getFullYear()} {clinicName || 'Klinik Alwi'}. All rights reserved.
      </div>
    </div>
  );
};
